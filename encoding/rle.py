...
def encode(x, word_length, scaling_factor, zero_point, l_bits, max_process_length=2 ** 32):
    ...
    x_flatten_total = x_quant.flatten()
    combined_total = torch.tensor([], device=x.device)

    # split into chunks to avoid memory overflow
    for i in range(0, len(x_flatten_total), max_process_length):
        x_flatten = x_flatten_total[i:min(i + max_process_length, len(x_flatten_total))]
        ...
        combined_total = torch.cat((combined_total, combined), dim=0)
