...
from models.object_detection.coco import UltralyticsModelWrapper
from quantization.utils import QuantMode, quantize_model

def main():
    parser = argparse.ArgumentParser(description='PyTorch COCO')
    parser.add_argument('--data', metavar='DIR', default="~/dataset/ultralytics/datasets",
                        help='path to dataset')
    parser.add_argument('-a', '--arch', metavar='ARCH', default='yolov8n',
    ...
    os.environ['COCO_PATH'] = os.path.expanduser(args.data)
    model_wrapper = UltralyticsModelWrapper(args.arch)
